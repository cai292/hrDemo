  ...
  <div v-loading="loading" class="dashboard-container">
    <div class="app-container">
      ...
      <page-tools :show-before="true">
        <!-- 前面内容 -->
        <template v-slot:before>当前审批中 {{ approveIn.length }} 本月审批通过 {{ approvepass.length }} 本月审批驳回 {{ approvereject.length }}</template>
...
export default {
  ...
  methods: {
    ...
    async  getApprovalList() {
      ...
      this.list = rows
      // this.list = rows.map(item => { return { ...item, procData: JSON.parse(item.procData) } })
      // console.log(this.page.total)
      // console.log(this.list)
      this.approveIn = this.list.filter(item => item.processState === '1')
      this.approvepass = this.list.filter(item => item.processState === '2')
      this.approvereject = this.list.filter(item => item.processState === '3')
      this.loading = false
    },
    toDetail(obj) {
