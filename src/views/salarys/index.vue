...
export default {
  ...
  computed: {
    tipsInfo() {
      ...
      return `本月9：入职 2 离职 5 调薪 4 未定薪 3`
    }
  },
  created() {
  ...
  methods: {
    ...
    formatEmployment(row) {
      // EmployeeData
      console.log(row)
      const data = EmployeeData.hireType.find(item => item.id === row.formOfEmployment)
    ...
    async  getDepartments() {
      ...
      this.departments = depts.splice(1)
      console.log(this.departments)
    },
    async getTips() {
      const { tipsRes } = await getTips(this.yearMonth)
      this.tips = tipsRes
