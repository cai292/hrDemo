<template>
  ...
  <div class="user-info">
    ...
    <el-form ref="form" :model="userInfo" label-width="220px">
      <el-row class="inline-info">
        ...
        <el-col :span="12">
          <el-form-item label="入职时间">
            ...
            <el-date-picker v-model="userInfo.timeOfEntry" type="date" placeholder="选择日期" />
          </el-form-item>
        </el-col>
      </el-row>
      <el-row class="inline-info">
      ...
      <el-row class="inline-info">
        ...
        <el-col :span="12">
          <el-form-item label="聘用形式">
            <el-select v-model="userInfo.formOfEmployment" placeholder="聘用形式" class="inputW">
...
export default {
  ...
  methods: {
    ...
    async getPersonalDetail() {
      this.formData = await getPersonalDetail(this.id)
      //   console.log(res)
      //   this.formData = res
      //   console.log(this.formData)
      // 获取完数据, 填充完表单, 应该将图片地址放入到对应的上传组件当中
      //   用户详情的图片
      if (this.formData.staffPhoto) {
        this.$refs.userInfoPhoto.fileList = [{ url: this.formData.staffPhoto }]
      }
    },
    // 保存更新基本信息
    async savePersonal() {
      ...
      await updatePersonal({ ...this.formData, staffPhoto: fileList[0] ? fileList[0].url : '', id: this.userId })
      this.$message.success('保存成功')
      await this.$store.dispatch('user/getUserInfo')// 用户资料
    }
  }
